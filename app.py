...
try:
    ...
    print(f"[ERROR] PROLOG engine failed to load: {e}")
...
def diagnose(image_path, crop_type, growth_stage, weather):
    ...
    if not expert_loaded:
        return "❌ **PROLOG Engine Error:** SWI-Prolog is missing or `knowledge_base.pl` is not found."
...
with gr.Blocks(title="Smart Crop Disease Detector", theme=gr.themes.Base()) as demo:
    gr.Markdown("# 🌿 Smart Crop Disease Detector\nUpload a leaf photo, select the farm conditions, and get a treatment plan powered by a CNN + PROLOG.")
